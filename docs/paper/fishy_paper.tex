...
\begin{tikzpicture}[
  box/.style={rectangle, rounded corners=3pt, draw=black!70, fill=gray!10,
              text width=2.0cm, align=center, font=\scriptsize, minimum height=0.65cm},
  decision/.style={diamond, draw=black!70, fill=yellow!15, aspect=2.0,
...
% Main horizontal chain (left to right)
...
% Branch up/down from decision
\node[box, above right=0.4cm and 0.4cm of kcheck] (pairwise) {Pairwise\\baseline\\stats};
\node[box, below right=0.4cm and 0.4cm of kcheck] (qsplit)   {Quarter-\\split\\(fallback)};

% Merge back
\node[box, right=2.6cm of kcheck] (bpa)    {BPA\\construction};
...
\draw[arr] (kcheck) -- node[lbl, above left=-1pt]{yes} (pairwise);
\draw[arr] (kcheck) -- node[lbl, below left=-1pt]{no}  (qsplit);
...
% Sublabels
\node[lbl, below=0.1cm of extract] {6 methods in parallel};
\node[lbl, above=0.08cm of bpa]    {empirical CDF};
\node[lbl, below=0.08cm of bpa]    {sigmoid};
