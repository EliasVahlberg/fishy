...
\begin{tikzpicture}[
  box/.style={rectangle, rounded corners=2pt, draw=black, fill=gray!10,
              text width=1.45cm, align=center, font=\tiny\bfseries, minimum height=0.6cm},
  decision/.style={diamond, draw=black, fill=yellow!20, aspect=1.8,
                   align=center, font=\tiny\bfseries, inner sep=0pt},
  arr/.style={-{Stealth[length=3.5pt]}, thick, draw=black},
  lbl/.style={font=\tiny\bfseries, text=black},
  node distance=0.28cm and 0.32cm
]

\node[box] (load)    {Load\\baselines\\+ test};
\node[box, right=of load]    (match)   {Source\\matching\\(SO/MO)};
\node[box, right=of match]   (extract) {Extract\\Repres-\\entations};
\node[box, right=of extract] (gate)    {Applicability\\gate\\($\hat{H}_M$)};
\node[decision, right=of gate] (kcheck) {$K{\geq}3$?};

\node[box, above right=0.35cm and 0.32cm of kcheck] (pairwise) {Pairwise\\baseline\\stats};
\node[box, below right=0.35cm and 0.32cm of kcheck] (qsplit)   {Quarter-\\split};

\node[box, right=2.2cm of kcheck] (bpa)    {BPA\\construct};
\node[box, right=of bpa] (ds)     {DS\\fusion};
...
\draw[arr] (kcheck) -- node[lbl, above left=-1pt]{\tiny yes} (pairwise);
\draw[arr] (kcheck) -- node[lbl, below left=-1pt]{\tiny no}  (qsplit);
...
\end{tikzpicture}
\caption{Adaptive fusion pipeline. Six analysis methods run in parallel after applicability gating. With $K \geq 3$ baselines, BPAs use empirical CDF of pairwise divergences; with one baseline, sigmoid z-score mapping is used as fallback.}
\label{fig:pipeline}
\end{figure*}
