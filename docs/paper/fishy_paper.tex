...
\begin{tikzpicture}[
  crate/.style={rectangle, rounded corners=4pt, draw=black!70, fill=gray!10,
                text width=5.8cm, minimum height=0.9cm, align=center, font=\small\bfseries},
...
\node[detail, right=0.15cm of enc] (encd) {};
\node[below=0.08cm of enc, detail] {Drain parse tree · timestamp detection · \texttt{TemplateId} dictionary};

\node[crate, fill=green!10, below=0.7cm of enc] (fis) {\texttt{fishy}};
\node[below=0.08cm of fis, detail] {detect() · adaptive fusion · multi-baseline · CLI};

\node[crate, fill=orange!10, below=0.7cm of fis] (ana) {\texttt{analysis}};
\node[below=0.08cm of ana, detail] {JSD · MI matrix · FFT · DWT · Laplacian · DS · BPA};

\draw[arr] (enc.south) -- node[right, detail]{JSON collections} (fis.north);
\draw[arr] (fis.south) -- node[right, detail]{math calls} (ana.north);

\node[detail, left=0.2cm of enc, align=right] {raw logs $\rightarrow$};
\node[detail, right=0.2cm of fis, align=left] {$\leftarrow$ AnomalyReport};
