...
\centering
\begin{tikzpicture}[
  crate/.style={rectangle, rounded corners=4pt, draw=black!80, minimum width=3.2cm,
                minimum height=0.65cm, align=center, font=\small\bfseries},
  mod/.style={rectangle, draw=black!40, fill=gray!8, minimum width=2.8cm,
              minimum height=0.5cm, align=center, font=\scriptsize},
  arr/.style={-{Stealth[length=4pt]}, thick},
  grp/.style={rectangle, rounded corners=6pt, draw=black!30, dashed, inner sep=6pt}
]

\node[crate, fill=blue!12]  (enc) at (0, 3.2)  {encoder};
\node[crate, fill=green!12] (fis) at (0, 1.8)  {fishy};
\node[crate, fill=orange!12](ana) at (0, 0.4)  {analysis};

\node[mod, right=0.4cm of enc] (drain)  {DrainTree};
\node[mod, right=0.4cm of fis] (detect) {detect()};
\node[mod, right=0.4cm of ana] (math)   {pure math};

\node[mod, left=0.4cm of enc]  (raw)    {raw logs};
\node[mod, left=0.4cm of fis]  (coll)   {LogCollection};
\node[mod, left=0.4cm of ana]  (types)  {BPA, MIMatrix\ldots};

\draw[arr] (enc) -- node[right, font=\scriptsize]{JSON collections} (fis);
\draw[arr] (fis) -- node[right, font=\scriptsize]{calls} (ana);

\end{tikzpicture}
\caption{Three-crate workspace. \texttt{analysis} is domain-agnostic (no \texttt{LogCollection} imports). \texttt{fishy} orchestrates the fusion pipeline. \texttt{encoder} tokenizes raw logs into the collection format.}
\label{fig:arch}
\end{figure}
